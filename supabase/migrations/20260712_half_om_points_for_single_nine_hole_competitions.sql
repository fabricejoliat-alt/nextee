-- Half OM points for single-round 9-hole stroke-play competitions.
-- Rule: only rounds that are truly played over 9 holes (1 x 9) should award
-- half the normal OM points. A 9-hole course played as 18 holes (2 x 9)
-- keeps full points.
...
begin
  ...
  v_occurred_on := (v_round.start_at at time zone 'Europe/Zurich')::date;
  v_coef := case
  ...
  if v_round.om_competition_format = 'match_play_individual' then
    ...
    select
      ...
      sum(h.score::numeric),
      count(*)
    into v_round_scores_count, v_round_total_score, v_round_holes_count
  ...
  insert into public.om_tournament_scores (
    ...
    total_points_brut, total_points_net, calculated_at, occurred_on
  )
  values (
    ...
    v_total_brut, v_total_net, now(), v_occurred_on
  )
  on conflict (round_id, organization_id) do update set
...
-- Recompute existing single-round 9-hole stroke-play competition rows so the
-- rankings become correct after the migration.
do $$
declare
  v_round_id uuid;
begin
  for v_round_id in
    ...
    from public.golf_rounds gr
    join public.golf_round_holes gh on gh.round_id = gr.id
    where gr.round_type = 'competition'
      and gr.om_competition_format = 'stroke_play_individual'
      and coalesce(gr.om_rounds_18_count, 1) = 1
    group by gr.id
    having count(*) = 9
  loop
    perform public.om_recompute_round(v_round_id);
  end loop;
end $$;
