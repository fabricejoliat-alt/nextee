-- Deleting a coach group must remove its group-scoped message threads first.
-- Otherwise message_threads.group_id would be set to NULL by FK on delete,
-- which violates message_threads_context_check for thread_type = 'group'.
