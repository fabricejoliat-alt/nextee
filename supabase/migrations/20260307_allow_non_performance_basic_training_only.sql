-- Non-performance players can log basic trainings,
-- but cannot submit structure/evaluation fields.
...
begin
  ...
  if not public.is_user_performance_enabled_globally(new.user_id) then
    if new.motivation is not null
       ...
       or new.satisfaction is not null
       or nullif(trim(coalesce(new.notes, '')), '') is not null then
