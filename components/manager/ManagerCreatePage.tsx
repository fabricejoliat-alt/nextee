...
export default function ManagerCreatePage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const json = await response.json();
      if (!response.ok) throw new Error(json.error ?? "Impossible de créer le manager.");
      router.push("/manager/user-management/managers");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Impossible de créer le manager.");
    } finally {
      setSaving(false);
    }
  }
