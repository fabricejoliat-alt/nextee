...
type Props = {
  ...
  quizAttempt: RulesQuizAttempt | null;
...
export default function PlayerRulesWorkspace({ series, current, cards, read, phase, phaseLabel, quizAvailable, quizAttempt, leaderboard }: Props) {
  const { locale } = useI18n();
  ...
  const openCard = useCallback((card: Card) => {
    ...
    listPosition.current = { windowY: window.scrollY, scrollAreaY: scrollArea?.scrollTop ?? 0 };
    setSelected(card);
  }, []);
  const closeCard = useCallback(() => {
    ...
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        ...
        const scrollArea = document.querySelector<HTMLElement>(".player-scroll-area");
        if (scrollArea) scrollArea.scrollTop = position.scrollAreaY;
      });
    });
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const scrollArea = document.querySelector<HTMLElement>(".player-scroll-area");
    const previous = {
      scrollAreaOverflow: scrollArea?.style.overflow ?? "",
      scrollAreaOverscrollBehavior: scrollArea?.style.overscrollBehavior ?? "",
    };
    if (scrollArea) {
    ...
    return () => {
      if (scrollArea) {
        scrollArea.style.overflow = previous.scrollAreaOverflow;
        scrollArea.style.overscrollBehavior = previous.scrollAreaOverscrollBehavior;
      }
    };
  }, [selected]);
  ...
  const quizSubmitted = quizAttempt?.status === "submitted";
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.cardsSection} aria-labelledby="cards-title"><div className={styles.sectionHeading}><div><h2 id="cards-title">{tr("À découvrir maintenant", "Explore now")}</h2><p>{tr("Les fiches de la série", "This series’ cards")}</p></div></div>
      {cards.length ? <div className={styles.cardGrid}>{cards.map(card => <button className={styles.card} key={card.card_version_id} onClick={() => openCard(card)} type="button">
    ...
    <section className={styles.calendarSection} aria-labelledby="calendar-title"><div className={styles.sectionHeading}><div><h2 id="calendar-title">{tr("Mon parcours", "My journey")}</h2><p>{tr("Les séries passées restent accessibles pour le rattrapage", "Past series remain available for catch-up")}</p></div></div><div className={styles.calendarGrid}>{series.map(item => <Link href={`/player/rules?series_id=${encodeURIComponent(item.id)}`} key={item.id} className={`${styles.calendarItem} ${item.id === current?.id ? styles.calendarCurrent : ""}`}><span className={styles.calendarNumber}>{String(item.position).padStart(2, "0")}</span><span className={styles.calendarText}><small>{month(item.discovery_starts_at)}</small><strong>{item.title_i18n[locale] ?? item.title_i18n.fr}</strong></span>{item.id === current?.id ? <span className={styles.calendarNow}>{tr("Affichée", "Selected")}</span> : <CalendarDays size={16} className={styles.calendarIcon} />}</Link>)}</div></section>

    {leaderboard && <section className={styles.leaderboardSection} aria-labelledby="leaderboard-title">
      <div className={styles.sectionHeading}><div><h2 id="leaderboard-title">{tr("Classement de la saison", "Season rankings")}</h2><p>{tr("Maîtrise en pourcentage, avec la participation toujours visible", "Percentage mastery with participation always visible")}</p></div></div>
      <div className={styles.leaderboardGrid}>
        {leaderboard.status === "published" ? leaderboard.clubs.map(club => <article className={styles.leaderboardCard} key={club.clubId}><div className={styles.leaderboardHeader}><span className={styles.leaderboardIcon}><Medal size={19} /></span><div><h3>{club.name ?? tr("Mon club", "My club")}</h3><p>{tr("Classement des joueurs du club", "Club player ranking")}</p></div></div>
          {club.rows.length ? <ol className={styles.leaderboardList}>{club.rows.map((row, index) => <li key={`${row.rank ?? "provisional"}-${index}`} className={row.isMe ? styles.leaderboardMe : ""}><span className={styles.leaderboardRank} title={!row.eligible ? tr("Classement provisoire", "Provisional ranking") : undefined}>{row.rank ? `#${row.rank}` : tr("PROV.", "PROV.")}</span><strong>{row.isMe ? tr("Vous", "You") : row.name ?? tr("Joueur", "Player")}<small>{row.eligible ? tr(`${row.completedSeries}/${leaderboard.publishedSeries} séries · ${points(row.rawPoints)} pts`, `${row.completedSeries}/${leaderboard.publishedSeries} series · ${points(row.rawPoints)} pts`) : tr(`Provisoire · ${row.completedSeries}/${row.minimumSeries} séries minimum`, `Provisional · ${row.completedSeries}/${row.minimumSeries} series minimum`)}</small></strong><span className={styles.leaderboardScore}>{points(row.score)} <small>%</small></span></li>)}</ol> : <p className={styles.leaderboardEmpty}>{tr("Aucun résultat publié dans ce club pour cette saison.", "No published result in this club for this season.")}</p>}
        </article>) : <article className={styles.leaderboardCard}><div className={styles.leaderboardHeader}><span className={styles.leaderboardIcon}><Medal size={19} /></span><div><h3>{tr("Dans mes clubs", "In my clubs")}</h3><p>{tr("Classement des joueurs", "Player ranking")}</p></div></div><p className={styles.leaderboardEmpty}>{leaderboardMessage}</p></article>}
        <article className={styles.leaderboardCard}><div className={styles.leaderboardHeader}><span className={styles.leaderboardIcon}><Trophy size={19} /></span><div><h3>{tr("Entre les clubs", "Between clubs")}</h3><p>{tr("Moyenne des pourcentages des juniors éligibles", "Average percentage of eligible juniors")}</p></div></div>
          {leaderboard.status === "published" ? leaderboard.interclub.rows.length ? <ol className={styles.leaderboardList}>{leaderboard.interclub.rows.map(row => <li key={row.clubId} className={row.isMyClub ? styles.leaderboardMe : ""}><span className={styles.leaderboardRank}>{row.rank ? `#${row.rank}` : "—"}</span><strong>{row.name}{row.isMyClub ? ` · ${tr("un de mes clubs", "one of my clubs")}` : ""}<small>{row.eligible ? tr(`${row.eligiblePlayers} juniors éligibles`, `${row.eligiblePlayers} eligible juniors`) : tr(`${row.eligiblePlayers}/${row.minimum} juniors éligibles · non classé`, `${row.eligiblePlayers}/${row.minimum} eligible juniors · unranked`)}</small></strong><span className={styles.leaderboardScore}>{row.eligible ? <>{points(row.score)} <small>%</small></> : "—"}</span></li>)}</ol> : <p className={styles.leaderboardEmpty}>{tr("Aucun club classé pour cette saison.", "No club ranked for this season.")}</p> : <p className={styles.leaderboardEmpty}>{leaderboardMessage}</p>}
        </article>
      </div>
    </section>}
