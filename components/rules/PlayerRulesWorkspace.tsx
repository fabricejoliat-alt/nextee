...
export default function PlayerRulesWorkspace({ series, current, cards, read, phase, phaseLabel, quizAttempt, leaderboard }: Props) {
  const { locale } = useI18n();
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.feature} aria-labelledby="current-series-title">
      <div className={styles.featureCopy}>
        ...
        <div className={styles.featureBottom}><span className={styles.phase}><Clock3 size={15} />{phaseLabel}</span><span>{cards.length} {tr("fiches disponibles", "cards available")}</span>{(quizOpen || quizSubmitted) && <Link className={styles.quizLink} href="/player/rules/quiz">{quizSubmitted ? <><Eye size={16} />{tr("Afficher mon quiz", "View my quiz")}</> : <>{tr("Faire le quiz", "Take the quiz")}<ArrowRight size={16} /></>}</Link>}</div>
      </div>
