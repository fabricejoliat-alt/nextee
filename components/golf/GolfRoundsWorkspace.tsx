...
function RoundDetail({ round, holes }: { round: Round; holes: Hole[] }) {
  ...
  const comparable = holes.filter((hole) => hole.par != null && hole.score != null);
  ...
  const scoreOption = useMemo<EChartsOption>(() => ({
    ...
    grid: { left: 35, right: 12, top: 20, bottom: 30 }, tooltip: { trigger: "axis", formatter: (params: unknown) => {
      const index = (params as Array<{ dataIndex?: number }>)?.[0]?.dataIndex ?? 0; const hole = comparable[index];
      return hole ? `Trou ${hole.hole_no}<br/>Par ${hole.par} · Score ${hole.score}<br/><b>${scoreToParLabel((hole.score as number) - (hole.par as number))}</b>` : "";
    } },
    xAxis: { type: "category", data: comparable.map((hole) => String(hole.hole_no)), axisLabel: { color: "#657168" } },
    yAxis: { type: "value", minInterval: 1, axisLine: { show: false }, splitLine: { lineStyle: { color: "#edf0ed" } } },
    series: [{ type: "bar", data: comparable.map((hole) => (hole.score as number) - (hole.par as number)), itemStyle: { color: (p: { dataIndex: number }) => comparable[p.dataIndex]?.hole_no <= 9 ? MANAGEMENT_CHART_COLORS[1] : MANAGEMENT_CHART_COLORS[2], borderRadius: 3 }, markLine: { silent: true, symbol: "none", lineStyle: { color: MANAGEMENT_CHART_COLORS[0] }, data: [{ yAxis: 0 }] } }],
  }), [comparable]);
  ...
  return <div className={styles.detail} onClick={(event) => event.stopPropagation()}>
    ...
    <div className={styles.charts}>
      {comparable.length >= 3 ? <section className={`${trainingStyles.panel} ${styles.chartCard}`}><h3>Écart au par par trou</h3><ActiviteeEChart option={scoreOption} ariaLabel="Évolution de l'écart au par trou par trou" height={230} /></section> : null}
...
export default function GolfRoundsWorkspace() {
  ...
  const [period, setPeriod] = useState<Period>("3m"); const [course, setCourse] = useState("all"); const [format, setFormat] = useState("all"); const [status, setStatus] = useState("all");
  ...
  const reset = () => { setPeriod("3m"); setCourse("all"); setFormat("all"); setStatus("all"); };
